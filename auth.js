...
// Import config (users will need to set this up)
...
class AuthManager {
  constructor() {
    ...
    if (this.checkFirebaseConfig(firebaseConfig)) {
      try {
      ...
      console.log('To enable cloud features, configure firebase-config.js');
    }
  }
  ...
  checkFirebaseConfig(config) {
    ...
    if (config.apiKey.includes('YOUR_') || config.projectId.includes('YOUR_')) {
      return false;
    }
    return true;
  }
  ...
  async signUp(email, password, displayName) {
    if (!this.isConfigured) {
      return { success: false, error: 'Firebase not configured. See SETUP_GUIDE.md' };
    }
    try {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const updatedStats = {
          ...
          bestStreak: won ? Math.max(currentStats.currentStreak + 1, currentStats.bestStreak) : currentStats.bestStreak,
          bestTimes: {
        ...
        if (won && gameData.time > 0) {
          if (!updatedStats.bestTimes[difficulty] || gameData.time < updatedStats.bestTimes[difficulty]) {
  ...
  async saveGameToHistory(gameData) {
    ...
    try {
      const gamesRef = collection(this.db, 'users', this.currentUser.uid, 'games');
  ...
  async getLeaderboard(limitCount = 10) {
    ...
    try {
      ...
      const q = query(usersRef, orderBy('stats.gamesWon', 'desc'), limit(limitCount));
      const querySnapshot = await getDocs(q);
      ...
      querySnapshot.forEach((doc) => {
        ...
        leaderboard.push({
          ...
          winRate: data.stats.gamesPlayed > 0 
            ? ((data.stats.gamesWon / data.stats.gamesPlayed) * 100).toFixed(1) 
            : 0
        });
      });
